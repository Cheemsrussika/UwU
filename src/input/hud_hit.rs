...
impl HudZone {
    ...
    pub fn get_clicked_inventory_slot(mx: f32, my: f32, sw: f32, sh: f32) -> Option<usize> {
        ...
        for slot in 0..46 {
            if let Some((gx, gy)) = crate::render::hud_ui::slot_coords::get_slot_pos(slot) {
    ...
    pub fn get_clicked_hotbar_slot(mx: f32, my: f32, sw: f32, sh: f32) -> Option<usize> {
        ...
        if ndc_y >= hb_y && ndc_y <= hb_y + total_h && ndc_x >= hb_x && ndc_x <= hb_x + total_w {
            let slot = ((ndc_x - hb_x) / slot_w).floor() as usize;
            if slot < 9 {
                return Some(slot);
            }
        }
