...
pub fn execute_render_passes(r: &mut Renderer) -> Result<(), wgpu::SurfaceError> {
    ...
    // 1. Shadow Pass
    {
        ...
        if r.world_index_count > 0 {
            sp.set_vertex_buffer(0, r.world_vertex_buffer.slice(..));
            sp.set_index_buffer(r.world_index_buffer.slice(..), wgpu::IndexFormat::Uint32);
            sp.draw_indexed(0..r.world_index_count, 0, 0..1);
        }
        if r.player_index_count > 0 {
            sp.set_vertex_buffer(0, r.player_vertex_buffer.slice(..));
            sp.set_index_buffer(r.player_index_buffer.slice(..), wgpu::IndexFormat::Uint32);
            sp.draw_indexed(0..r.player_index_count, 0, 0..1);
        }
    }
    ...
    // 2. 3D Scene Pass
    {
        let mut mp = enc.begin_render_pass(&wgpu::RenderPassDescriptor {
            ...
            color_attachments: &[Some(wgpu::RenderPassColorAttachment {
                ...
                ops: wgpu::Operations { load: wgpu::LoadOp::Clear(wgpu::Color { r: 0.1, g: 0.12, b: 0.16, a: 1.0 }), store: wgpu::StoreOp::Store },
            })],
            depth_stencil_attachment: Some(wgpu::RenderPassDepthStencilAttachment {
        ...
        if r.world_index_count > 0 {
            mp.set_vertex_buffer(0, r.world_vertex_buffer.slice(..));
            mp.set_index_buffer(r.world_index_buffer.slice(..), wgpu::IndexFormat::Uint32);
            mp.draw_indexed(0..r.world_index_count, 0, 0..1);
        }
        if r.player_index_count > 0 {
            mp.set_vertex_buffer(0, r.player_vertex_buffer.slice(..));
            mp.set_index_buffer(r.player_index_buffer.slice(..), wgpu::IndexFormat::Uint32);
            mp.draw_indexed(0..r.player_index_count, 0, 0..1);
        }

        // Overlay with depth_compare: Always
